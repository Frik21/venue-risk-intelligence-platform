...
// the outline - so this is a genuinely separate asset (generated offline,
// see generate-night-map-no-lights.py alongside it), not a CSS filter or
// a conditional unmount of the whole layer. There's no clean brightness
// threshold that separates "faint glow falloff" from "unlit landmass
// texture" (sampling the source image shows a continuous gradient
// between them), so the generator detects glow adaptively against each
// pixel's own local surroundings rather than a fixed global cutoff -
// see that script's own comment for why. This remains an approximation,
// not a pixel-perfect reconstruction of unlit terrain.
const NIGHT_MAP_NO_LIGHTS_URL = `${import.meta.env.BASE_URL}data/earth-at-night-no-lights.png`;
...
function CountrySelect({
  ...
  // marker, the 1.1 zoom-out control, and the 1.3 lights-removed night-map
  // swap, not a separate timer. MapLock's own imperative dragging.disable()
  // call still runs once on mount (matching the default world view's
  // static lock); this effect is the only thing that re-enables it, and
  // only for the duration of a country selection. No maxBounds is set
...
function MapLayer() {
  ...
  // and the 1.3 lights-removed night-map swap below, all off this same
  // timing so they change state together. A timer matching
