...
- **"Task Pending Quotation" placeholder**: added as an empty ("Coming soon") card on the Quotations page (`costs.tsx`), between the Quotes ledger and the Quotation Workspace, per direct product direction to leave it blank for now. Scope not yet defined - come back to this once it's decided whether it should list tasks with no `estimatedCost` set, tasks with no linked formal Quote, or something else.
